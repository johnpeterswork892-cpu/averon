...
export async function POST(req: Request) {
  try {
    ...
    if (!trackingRecord) {
      return NextResponse.json(
        { success: false, error: 'Invalid tracking number. It was not generated by the system.' },
        { status: 400 }
      );
    }
    ...
    // Check if a shipment with this tracking number already exists
    const existing = await Shipment.findOne({ trackingNumber });
    if (existing) {
      return NextResponse.json(
        { success: false, error: 'A shipment with this tracking number already exists' },
        { status: 409 }
      );
    }
